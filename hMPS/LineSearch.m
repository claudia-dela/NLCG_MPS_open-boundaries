...
% Output: 
% alpha - lenght step from A along dir
%         which minimize the funtional value

function alpha = nocLineSearch(N,W,B,x,dir,slope0,of,nstep)
...
while 1
    ...
    XC = BuildA(B,xc);
    of     = ExpVal(N,W,XC);
    slopec = conj(MPSgrad(N,W,XC,B))*dir.';
    
    if  (of > of_0 + slope0*c1*alpha_1) || ((of >= of_x ) && (iter > 0))
        alpha = Zoom(N,W,B,x,dir,slope0, alpha_0, alpha_1,of_0,of_x ,c1,c2,nstep);
        break;
    end
    ...
    if (slopec >= 0)
        alpha = Zoom(N,W,B,x,dir,slope0,alpha_1 , alpha_0,of_0,  of,c1,c2,nstep);
        break;
    end
    alpha_0 = alpha_1;
...
function alpha = Zoom(N,W,B,x,dir,slope0,alphaLo,alphaHi,of_0,ofLo,c1,c2,nstep)
for i=1:nstep 
