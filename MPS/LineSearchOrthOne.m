...
function alpha = LineSearchOrthOne(N,W,BORTH,A,dir,slope0,of,nstep)
	...
	while 1
		...
		of = ExpVal(W,XC);
    		slopec = conj(MPSgrad(W,XC,BORTH))*dir.';
    
    		if  (of > of_0 + slope0*c1*alpha_1) || ((of >= of_x ) && (iter > 0))
        		alpha = Zoom(N,W,BORTH,A,dir,slope0, alpha_0, alpha_1,of_0,of_x ,c1,c2,nstep);
        		break;
    		end
	    	...
	    	if (slopec >= 0)
			alpha = Zoom(N,W,BORTH,A,dir,slope0,alpha_1 , alpha_0,of_0,  of,c1,c2,nstep);
			break;
	    	end
	    	alpha_0 = alpha_1;
...
function alpha = Zoom(N,W,BO,A,dir,slope0,alphaLo,alphaHi,of_0,ofLo,c1,c2,nstep)
	for i=1:nstep 
		alpha = (alphaLo+alphaHi)/2;
    		...
    		if of > of_0 + c1*alpha*slope0 || of >= ofLo
        		...
        		slopec = conj(MPSgrad(W,XC,BO))*dir.';
        		if abs(slopec) <= -c2*slope0
            			return;
        		end
